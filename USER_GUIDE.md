...
| `--report-include-valid` | off | `report.json` 的 `teich_trace_results` 仅写入 valid 文件（默认空数组） |
| `--quiet` | off | 静默 |
...
| `teich_trace_results` | 逐条 Teich 校验明细（默认 `[]`；`--report-include-valid` 时仅含 valid） |
| `valid_files_omitted` | 未写入 `teich_trace_results` 的 valid 条数（仅在使用 `--report-include-valid` 时不出现） |
| `scanned` | 扫描行数 |
...
`teich_trace_results` 每项（仅 `--report-include-valid` 时）：`filename`、`session_key`、`status`、`error`、`trace_type`、`messages_count`、`tools_count`、`last_relevant_role`。

incomplete / invalid 逐文件结果不写入报表，见输出目录 `incomplete/`、`invalid/`。
...
- Teich 汇总（判定标准、`teich_errors`、明细目录指引）
...
- [ ] 若有 incomplete：查看 `incomplete/` 目录或 `report.json` 中 `teich_incomplete` 汇总计数
...
A：R5 检查单条 delivery response 的 `stop_reason`；Teich `trace_is_complete` 检查**整段转换后 messages** 的末条 relevant role。能通过 R5 的记录，仍可能因 Codex 转换后的消息序列以 `tool` 结尾而 incomplete（例如 session 最长快照仍停在工具链中间）。逐文件结果见 `incomplete/` 目录。
